...
`catalog.txt` lists all the songs in the library. The syntax is fairly strict
and designed to be parsed and written by a machine. The format is one song per
line. Blank lines are not permitted. Each line has the following format:
...
- `mhash` is the SHA256 hash of the MP3 data **excluding** the ID3 tag.

The `artist` and `title` fields may only include lowercase letters, digits,
