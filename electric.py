...
# This is my famous tag generator system.
...
def get_song_tags(song):
    return Tags(
        (SONG_TITLE_OVERRIDES[song.baseid] if song.baseid in SONG_TITLE_OVERRIDES
            else song.title.upper().replace('-', ' ')),
        (ARTIST_NAME_OVERRIDES[song.artist] if song.artist in ARTIST_NAME_OVERRIDES
            else song.artist.upper().replace('-', ' ')),
        song.group.album)
...
def command_update(context, args):
    ...
    for song in manifest.songs():
        tags = song.tags()
        if song.tag_hash == get_tag_hash(tags):
        ...
        print(f"  artist: {tags.artist}")
        print(f"  album: {tags.album}")
        items.append(RetagItem(song, tags, os.path.join(context.root, song.basepath())))
    ...
    for item in items:
        ...
        dest_file = os.path.join(context.root, item.song.basepath())
...
def command_maint(context, args):
    ...
    for group in manifest.groups():
        ...
        for song in group.songs():
            ...
            filename = os.path.join(context.root, song.basepath())
            try:
